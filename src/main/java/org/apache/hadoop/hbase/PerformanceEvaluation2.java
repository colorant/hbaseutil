...
public class PerformanceEvaluation2 {
  ...
  protected void printUsage(final String message) {
    ...
    System.err.println(" --table=abc --cols=\"f:col1 f:col2\" [--cachesize=nline] [--filter=] [nThreads]");
    System.err.println(" <--type=native|mapred|mapred2> [--enablecacheblock] [--ignoreresult] [--rows=] [--repeat=ntimes]");
    System.err.println();
  }
  ...
  public int doCommandLine(final String[] args) {
    ...
    for (int i = 0; i < args.length; i++) {
      ...
      final String cachesize = "--cachesize=";
      if (cmd.startsWith(cachesize)) {
        optionCacheSzie = cmd.substring(cachesize.length());
        this.conf.set(Test.KEY_OPTION_CACHE_SIZE, optionCacheSzie);
